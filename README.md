...
Memos rot, but wikis compound. wiki-wizard implements the workflow Karpathy laid out in his "LLM Wiki" Gist. Every source becomes a `raw/` snapshot, a `wiki/summaries/` page, and 10 to 15 entity and concept page touches. Queries pull from this structured wiki rather than a flat file dump, so answers can cite specific pages. The act of querying tends to produce new syntheses, which file themselves back into the wiki and close the loop.
...
- Your files stay in the vault path you chose. wiki-wizard never touches them outside the op you explicitly invoked.
