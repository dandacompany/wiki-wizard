...
Your vaults and the registry (`~/.omw/registry.db`) are never touched.
...
- The vault registry lives at `~/.omw/registry.db` (override the root with `OMW_HOME`) as a per-user sqlite database.
